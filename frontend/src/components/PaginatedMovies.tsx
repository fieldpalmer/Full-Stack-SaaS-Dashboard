     ...
     const fetchMovies = async () => {
          try {
               ...
               const { data } = await axios.get(
                    `https://full-stack-saas-dashboard.onrender.com/api/movies?page=${page}&limit=10&sortField=${sortField}&sortOrder=${sortOrder}&genre=${genre}&rating=${rating}&year=${year}`,
     ...
     return (
          <div className='bg-gray-800 p-6 rounded-lg shadow-md mt-6'>
               ...
               <div className='flex flex-wrap gap-4 mb-4'>
                    <select onChange={(e) => setSortField(e.target.value)} className='bg-gray-700 p-2 rounded'>
                         <option value='year'>Sort by Year</option>
                         <option value='title'>Sort by Title</option>
                         <option value='runtime'>Sort by Runtime</option>
                    </select>
                    <select onChange={(e) => setSortOrder(e.target.value)} className='bg-gray-700 p-2 rounded'>
                    ...
                    <tbody>
                         {movies.map((movie, index) => (
                              <tr key={index} className='border-b border-gray-700 hover:bg-gray-700 transition'>
                                   ...
                                   <td className='py-2 px-4'>{movie.runtime ? `${movie.runtime} mins` : 'N/A'}</td>
